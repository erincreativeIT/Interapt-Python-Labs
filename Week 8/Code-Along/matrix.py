# Poor man's implementation of a 2-0 matrix class
import operator

class Matrix:
    '''
        data is a list. order is a tuple of (rows, columns)
        Maube explore additional arg checking?
    '''
    def __init__( self, data, order):
        # Should we check if 'order' is a tuple of two INTEGERS???
        if not all ([isinstance(elem, int) for elem in order]) or len(order) != 2:
            raise ValueError ("Dimensions passed to CTOR are not integral or two elements not passed for demensions ")
    ...
    def _operate_on_matrices(self, rho_mat, operation):
        # Check if rowss and columns of both match
...
if __name__ == "__main__":
    ...
    print("Matrix 'mat2':")
    print(mat2)
    # Add then, print result
    sum_mats = mat + mat2
    print("Sum of the above matrices:")
    print(sum_mats)
    diff_mats = sum_mats - mat
    print("Differences of the above matrices:")
    print(diff_mats)
